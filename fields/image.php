<div class="<?=$input_class != "" ? $input_class : '';?> <?=$the_id?> input-<?=$type;?> field_<?=$the_id?>">
    ...
    <input type="file" class="<?=(@$req != ""?'required':'');?> <?=$the_name?>_file btn btn-outline-secondary" id="<?=$the_id?>_file" name="<?=$the_name?>_file" <?=(@$req != "" ? 'required="required"' : '');?> accept="image/*" onChange="document.getElementById('<?=$the_id?>_preview').src = window.URL.createObjectURL(this.files[0]);document.getElementById('<?=$the_id?>_preview').style.display = 'block'"/>
